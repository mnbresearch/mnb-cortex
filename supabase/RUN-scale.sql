/* ===========================================================================
   MNB CORTEX — scale readiness. Run the whole file in the Supabase SQL editor.
   Safe to run more than once. Purely additive: no policy on tenant data
   changes here, no row is written, no result changes — only how fast things
   are found, and which workspaces the nightly cron reaches.
   =========================================================================== */
...
select 'cron_cursors' as check_name,
       ...
       coalesce((select string_agg(name, ', ') from cron_cursors), 'no rows') as detail;

...
export async function POST(req: Request) {
  ...
  if (agent.kind === "image") {
    ...
    /*
      Everything below the charge is wrapped, because everything below the
      charge can fail: the image provider is a third-party HTTP call and
      saveRun writes to the database. Before this, a provider timeout threw out
      of the route as an unhandled 500 with the credits already deducted — the
      customer saw a generic error page and a smaller balance.
    */
    try {
  ...
  try {
    ...
    if (!String(output || "").trim()) {
      ...
      return NextResponse.json({ ok: false, error: `"${agent.name}" came back empty — try running it again. Your credits have not been used.` }, { status: 200 });
    }
    ...
    await refundIfCharged(gate, "document");
    return NextResponse.json({ ok: false, error: (e?.message || `"${agent.name}" could not finish.`) + " Your credits have not been used." }, { status: 200 });
  }
}

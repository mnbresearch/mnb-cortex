...
import { upcomingDeadlines, whenPhrase, URGENT_WITHIN_DAYS, splitByProfile, STATUTORY_CATALOGUE, ruleWhen } from "@/lib/statutory";
...
/*
  BOTH REFERENCE TABLES COME FROM THE CATALOGUE.

  They used to be two arrays typed into this file, and they had drifted from
  lib/statutory.ts — the single source every other surface renders from. The
  drift was not cosmetic:

    · "ROC filings (AOC-4, MGT-7) — 30 Sep / 31 Oct", where the catalogue and
      the public /deadlines pages say AOC-4 is 30 Oct and MGT-7 is 29 Nov. A
      customer comparing the two screens got two different filing schedules.

    · "ITR + Tax Audit — 31 Oct" as one obligation, when the 44AB audit report
      is due a month before the return. Anyone planning from this line filed
      their audit report a month late.

  Rendering from STATUTORY_CATALOGUE means a date can only ever be wrong in one
  place, and fixing it there fixes every screen at once.
*/
const monthlyRules = STATUTORY_CATALOGUE.filter((r) => r.cadence === "monthly");
const annualRules = STATUTORY_CATALOGUE.filter((r) => r.cadence === "annual");

/* Derived from severity rather than assigned by hand, which is how the old
   table ended up colouring two equally-high obligations differently. */
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {monthlyRules.map((r) => (
              <Card key={r.id} className="p-4 flex items-start gap-3">
                <div className="h-10 w-10 rounded-lg brand-gradient grid place-items-center text-white font-bold shrink-0">{r.day}</div>
                <div><div className="flex items-center gap-2"><span className="font-medium text-sm">{r.name}</span><Badge className={tone[r.severity]}>{ruleWhen(r)}</Badge></div><div className="text-xs text-muted-foreground">{r.what}</div></div>
              </Card>
            ))}
          </div>
        </Section>
            ...
            {annualRules.map((r) => (
              <Card key={r.id} className="p-4 flex items-center gap-3">
                <div className="flex-1">
                  ...
                  {/* The condition travels with the date. A proprietor should never
                      read an AOC-4 row as something they have missed. */}
                  <div className="text-xs text-muted-foreground/80 mt-0.5">Applies if {r.appliesIf}.</div>
                </div>
                <Badge className={tone[r.severity]}>{ruleWhen(r)}</Badge>
              </Card>
            ))}
          </div>
        </Section>

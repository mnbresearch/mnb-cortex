    ...
    <>
      ...
      <PageShell>
        <Section title="Team members" desc={live ? "Live members of your workspace" : "Sign in to manage your real team"}>
          ...
          {live && (
            <SafeForm action={inviteMember} className="mt-4 flex flex-wrap items-end gap-2">
            ...
            <div className="mt-4 space-y-2">
              ...
              {invites.filter((i:any)=>i.status==="pending").map((i: any) => (
                <div key={i.id} className="flex items-center justify-between rounded-lg border border-dashed p-3 text-sm">
                  ...
                  <SafeForm action={cancelInvite}><input type="hidden" name="id" value={i.id} /><button className="text-muted-foreground hover:text-danger min-h-11 min-w-11 p-2" aria-label="Close"><X aria-hidden="true" className="h-4 w-4" /></button></SafeForm>
                </div>
              ))}
            </div>
          )}

...
export async function updateStatus(fd: FormData): Promise<ActionResult | void> {
  ...
  /*
    ...
    `table` is whitelisted above and `org_id` scopes the write, which is what
    makes this safe. What was missing is that a row matching NEITHER — a stale
    id from a page left open, a row another member already deleted, or an id
    belonging to a different workspace — updates nothing, returns no error, and
    then the three lines below run anyway: metrics are recomputed, the activity
    feed records "Set invoices to paid", and five paths are revalidated. So the
    audit trail gains an entry for a change that did not happen, which is worse
    than the silent no-op on its own.

    Reading the row back turns that into an error the user can act on.
  */
  const { data: changed, error } = await sb.from(table)
...
export async function approveMessage(fd: FormData): Promise<{ ok: boolean; error?: string }> {
  ...
  */
  /*
    AND THE FILTER EXCLUDING THE ROW IS NOT SUCCESS.

    The comment above explains exactly why `.eq("status","draft")` is here — so
    re-submitting cannot resurrect a sent message — and then the code reported
    `ok: true` when that filter matched nothing. A PostgREST UPDATE over zero
    rows returns no error, so the two cases the filter exists to separate were
    reported identically: "approved it" and "refused to approve it because it
    had already gone out".

    `.select("id")` makes the distinction readable. A double-clicked Approve,
    or an approve on a message the nightly run already sent, now says so
    instead of confirming an action that did not happen.
  */
  const { data: approved, error } = await sb.from("collection_messages")
    .update({ status: "approved", approved_at: new Date().toISOString() })
    .eq("id", id).eq("org_id", orgId).eq("status", "draft")
    .select("id");
  if (error) return { ok: false, error: error.message };
  if (!((approved as any[]) || []).length) {
    return { ok: false, error: "That message is no longer a draft — it may already be approved, sent or cancelled. Reload to see where it is." };
  }
  revalidatePath("/collections");
  return { ok: true };
}
...
export async function cancelMessage(fd: FormData): Promise<{ ok: boolean; error?: string }> {
  ...
  /*
    ...
    `.in("status", ["draft","approved"])` means a message that has already been
    SENT cannot be cancelled — correctly, because you cannot unsend a WhatsApp
    message to someone's customer. But a zero-row UPDATE returned no error, so
    the function answered `ok: true` and the UI confirmed the cancellation.

    So the customer pressed Cancel on a chase to one of their own debtors,
    was told it was cancelled, and it had already gone out — or worse, was
    still queued under a status this filter does not cover and went out
    afterwards. Either way they believed they had stopped something they had
    not, about a relationship they care about. That is the single most
    expensive thing a false success can do in this product.
  */
  const { data: cancelled, error } = await sb.from("collection_messages")
    .update({ status: "cancelled" })
    .eq("id", str(fd.get("id"))).eq("org_id", orgId).in("status", ["draft", "approved"])
    .select("id,status");
  if (error) return { ok: false, error: error.message };
  if (!((cancelled as any[]) || []).length) {
    return {
      ok: false,
      error: "That message could not be cancelled — it has most likely already been sent, which cannot be undone. Reload to see its current state.",
    };
  }
  revalidatePath("/collections");
  return { ok: true };

  ...
  return (
    <Card>
      <div className="flex items-center justify-between p-5 pb-3">
        ...
        <div className="flex gap-2"><ExportButton rows={rows} filename={`${table}.csv`} columns={cols.map((c) => c.key)} /><ExcelButton rows={rows} filename={`${table}.csv`} columns={cols.map((c) => c.key)} /></div>
      </div>
